...
!function(e, t) {
    ...
    const m = /^ {0,3}((?:-[\t ]*){3,}|(?:_[ \t]*){3,}|(?:\*[ \t]*){3,})(?:\n+|$)/
      ...
      , _ = k(/^( {0,3}bull)([ \t][^\n]+?)?(?:\n|$)/).replace(/bull/g, y).getRegex()
      , A = "address|article|aside|base|basefont|blockquote|body|caption|center|col|colgroup|dd|details|dialog|dir|div|dl|dt|fieldset|figcaption|figure|footer|form|frame|frameset|h[1-6]|head|header|hr|html|iframe|legend|li|link|main|menu|menuitem|meta|nav|noframes|ol|optgroup|option|p|param|section|source|summary|table|tbody|td|tfoot|th|thead|title|tr|track|ul"
      , S = /<!--(?!-?>)[\s\S]*?(?:-->|$)/
      ...
      , B = /^( {2,}|\\)\n(?!\s*$)/
      , Q = "\\p{P}$+<=>`^|~"
      , M = k(/^((?![*_])[\spunctuation])/, "u").replace(/punctuation/g, Q).getRegex()
      , O = k(/^(?:\*+(?:((?!\*)[punct])|[^\s*]))|^_+(?:((?!_)[punct])|([^\s_]))/, "u").replace(/punct/g, Q).getRegex()
      , C = k("^[^_*]*?__[^_*]*?\\*[^_*]*?(?=__)|[^*]+(?=[^*])|(?!\\*)[punct](\\*+)(?=[\\s]|$)|[^punct\\s](\\*+)(?!\\*)(?=[punct\\s]|$)|(?!\\*)[punct\\s](\\*+)(?=[^punct\\s])|[\\s](\\*+)(?!\\*)(?=[punct])|(?!\\*)[punct](\\*+)(?!\\*)(?=[punct])|[^punct\\s](\\*+)(?=[^punct\\s])", "gu").replace(/punct/g, Q).getRegex()
      , D = k("^[^_*]*?\\*\\*[^_*]*?_[^_*]*?(?=\\*\\*)|[^_]+(?=[^_])|(?!_)[punct](_+)(?=[\\s]|$)|[^punct\\s](_+)(?!_)(?=[punct\\s]|$)|(?!_)[punct\\s](_+)(?=[^punct\\s])|[\\s](_+)(?!_)(?=[punct])|(?!_)[punct](_+)(?!_)(?=[punct])", "gu").replace(/punct/g, Q).getRegex()
      , j = k(/\\([punct])/, "gu").replace(/punct/g, Q).getRegex()
      ...
      , G = k(/^!?\[(label)\]\[(ref)\]/).replace("label", F).replace("ref", R).getRegex()
      , J = k(/^!?\[(ref)\](?:\[\])?/).replace("ref", R).getRegex()
      , K = {
        _backpedal: f,
        anyPunctuation: j,
        autolink: H,
    ...
    class oe {
            ...
            return e.forEach((e=>{
                ...
                if (e.tokenizer) {
                    ...
                    for (const n in e.tokenizer) {
                        if (!(n in t) || ["options", "rules", "lexer"].includes(n))
        ...
        #e(e, t) {
            return (n,s)=>{
                ...
                i.async)
                    return Promise.resolve(i.hooks ? i.hooks.preprocess(n) : n).then((t=>e(t, i))).then((e=>i.walkTokens ? Promise.all(this.walkTokens(e, i.walkTokens)).then((()=>e)) : e)).then((e=>t(e, i))).then((e=>i.hooks ? i.hooks.postprocess(e) : e)).catch(l);
                try {
